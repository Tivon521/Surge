...
!(async () => {
  ...
  if (allMessage) {
    $.msg($.name, '', allMessage);
    if ($.isNode()) await notify.sendNotify(`${$.name}`, `${allMessage}`, { url: jxOpenUrl })
  }
})()
    .catch((e) => {
      $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
  ...
  if (exchangeEndTime) {
    ...
    if (nowTimes < exchangeEndTime) {
      ...
      if ((exchangeEndTime - nowTimes.getTime()) <= 3600000 * 4) {
        ...
        $.msg($.name, ``, `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}小时后兑换超时\n【截止时间】${$.exchangeEndTime}\n【兑换入口】京喜APP->我的->京喜工厂🏭`, {'open-url': jxOpenUrl, 'media-url': $.picture})
