/*
Last Modified time: 2022-1-21 18:22:37
      ...
      if (rewardNum && (rewardNum === 1 || rewardNum === 20 || rewardNum === 50 || rewardNum === 100 || rewardNum === 500 || rewardNum === 1000)) {
        ...
        if (salePrice) {
          if (leftStock) {
            ...
            await giftNewExchange(saleInfoId, 'pet');
...
function getExchangeRewards() {
  return new Promise(async (resolve) => {
    ...
    const time = Date.now()
    const h5st = await getH5stBody('giftGetBeanConfigs', $.toStr(body), time);
    const option = {
      url: `https://api.m.jd.com/api?client&clientVersion&appid=jdchoujiang_h5&t=${time}&functionId=giftGetBeanConfigs&body=${$.toStr(body)}&h5st=${h5st}`,
      headers: {
    ...
    $.post(option, (err, resp, data) => {
      try {
        if (err) {
          ...
          $.getExchangeRewardsRes = $.toObj(data, {});
        }
      } catch (e) {
        $.logErr(e, resp);
      } finally {
...
function giftNewExchange(saleInfoId, orderSource) {
  const body = {
    reqSource: 'h5',
    buyParam: {
  ...
  return new Promise(async (resolve) => {
    const time = Date.now()
    $.appId = '9218e'
    const h5st = await getH5stBody('giftNewExchange', $.toStr(body));
    const option = {
      url: `https://api.m.jd.com/api?client&clientVersion&appid=jdchoujiang_h5&t=${time}&functionId=giftNewExchange&body=${$.toStr(body)}&h5st=${h5st}`,
      headers: {
        "Content-Type": "application/json",
        "Cookie": cookie,
        "Connection": "keep-alive",
