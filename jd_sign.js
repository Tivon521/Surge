...
!(async () => {
  ...
  $.fingerprint = 8255954019893296 || await generateFp();
  $.token = '';
  await requestAlgo();
...
function Sign(i) {
  return new Promise(async resolve => {
    const body = {
      "turnTableId": `${turnTableId[i].id}`,
      "fp": fp,
      "lks": $.md5(invokeKey + $.lkt).toString(),
      "lkt": $.lkt,
      "invokeKey": invokeKey
    }
...
function Login(i) {
  return new Promise(async resolve => {
    const body = {"turnTableId": `${turnTableId[i].id}`, "invokeKey": invokeKey};
        ...
        if (err) {
          ...
          if (data) {
            ...
            if (data.success && data.data) {
              ...
              if (!data.hasSign) {
                console.log(`签到地址：${turnTableId[i].url}`)
...
async function getH5stBody(functionId = '', bodyInfo) {
  ...
  let url = `https://api.m.jd.com?appid=jdchoujiang_h5&functionId=${functionId}&body=${bodySign}&client=jxh5&clientVersion=1.2.5`;
