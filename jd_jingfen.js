...
其中：appKey（可选）和appSecret（可选）可在 https://www.dgrlm.com/open/index 申请
...
const version = 'v2';// 接口版本：v1（旧版本），v2（新版本），默认为v1版本，建议使用v2版本
...
async function changeLinks(urls) {
  ...
  for (let url of urls) {
    ...
    try {
      ...
      console.log(`当前商品：${data['info'].replace(/\r\n/, '')}不在推广中`);
    }
  }
  return finalUrls
}
