/*
活动地址：https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/activity/dz2111100000412301?activityId=dz2111100000412301&shareUuid=7646a08ca0704396a2bb57957a0c1afa
31 20 * * * jd_xiaomi.js
 */
const $ = new Env('小米双12');
...
!(async () => {
    ...
    $.shareUuid = '';
    let shareList = ['7646a08ca0704396a2bb57957a0c1afa'];
    if(shareList.length > 0){
        $.shareUuid = getRandomArrayElements(shareList,1)[0];
    }
    let activityList = [{'id':'dz2111100000412301','endTime':'1639065599000'}];
...
async function main() {
    ...
    Referer = `https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/activity/${activityID}?activityId=${activityID}&shareUuid=${$.shareUuid}`;
    ...
    if(!$.activityData.isFollowShop){
        await $.wait(2000);
        await takePostRequest('followShop');
    }
    await $.wait(2000);
    console.log(`去助力：${$.shareUuid}`);
    await takePostRequest('shareRecord');
    await $.wait(2000);
    await doTask()

}
async function doTask(){
    let taskInfo = $.activityData.activeTask;
    for (let oneType in taskInfo){
        let oneTask =  taskInfo[oneType];
        if(oneTask.allStatus){
            console.log(`任务：${oneType},已完成`);
            continue;
        }
        if(oneType === 'shareActive'){
            continue;
        }
        if(oneType === 'addSku'){
            $.taskType = 21;
            $.taskValue	= 21;
            console.log(`任务：${oneType},去执行`);
            await takePostRequest('saveTask');
            await $.wait(3000);
            continue;
        }
        let oneTaskList = oneTask.data;
        for (let i = 0; i < oneTaskList.length; i++) {
            let oneInfo = oneTaskList[i];
            $.taskType = oneInfo.type;
            $.taskValue	= oneInfo.value;
            console.log(`任务：${oneType},去执行`);
            if(oneInfo.status !== 0){
                continue;
            }
...
function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'activityContent':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/activityContent';
            ...
            break;
        case 'followShop':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/followShop';
            body = `activityId=${activityID}&actorUuid=${actorUuid}&pin=${encodeURIComponent(pin)}`;
            break;
        case 'shareRecord':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/shareRecord';
            body = `activityId=${activityID}&actorUuid=${actorUuid}&pin=${encodeURIComponent(pin)}&shareUuid=${$.shareUuid}`;
            break;
        case 'saveTask':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/saveTask';
...
function getToken() {
    let config = {
        ...
        body: 'body=%7B%22url%22%3A%22https%3A%5C/%5C/lzdz1-isv.isvjcloud.com%22%2C%22id%22%3A%22%22%7D&build=167874&client=apple&clientVersion=10.2.4&d_brand=apple&d_model=iPhone9%2C2&ef=1&eid=eidI42470115RDhDRjM1NjktODdGQi00RQ%3D%3DB3mSBu%2BcGp7WhKUUyye8/kqi1lxzA3Dv6a89ttwC7YFdT6JFByyAtAfO0TOmN9G2os20ud7RosfkMq80&ep=%7B%22ciphertype%22%3A5%2C%22cipher%22%3A%7B%22screen%22%3A%22CJS0CseyCtK4%22%2C%22osVersion%22%3A%22CJGkDq%3D%3D%22%2C%22openudid%22%3A%22DWO4YJU3DNDrDWGyYJGnCJLrEQVuCzu2YwSmDNc0DzPvYJOyCQC2YG%3D%3D%22%2C%22area%22%3A%22Cv8yENCmXzUnENS4XzK%3D%22%2C%22uuid%22%3A%22aQf1ZRdxb2r4ovZ1EJZhcxYlVNZSZz09%22%7D%2C%22ts%22%3A1638408072%2C%22hdid%22%3A%22JM9F1ywUPwflvMIpYPok0tt5k9kW4ArJEU3lfLhxBqw%3D%22%2C%22version%22%3A%221.0.3%22%2C%22appname%22%3A%22com.360buy.jdmobile%22%2C%22ridx%22%3A-1%7D&ext=%7B%22prstate%22%3A%220%22%7D&isBackground=N&joycious=89&lang=zh_CN&networkType=wifi&networklibtype=JDNetworkBaseAF&partner=apple&rfs=0000&scope=01&sign=242dfbfb45ef15c979fa2037e4690259&st=1638420411234&sv=101',
        headers: {
            'Host': 'api.m.jd.com',
            'accept': '*/*',
