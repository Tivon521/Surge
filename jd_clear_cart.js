/*
清空购物车（包含预约、预售商品，下架商品会删除失败）
默认不清空。如需：设置环境变量 JD_CART 为 true
15 9 * * * jd_clear_cart.js
 */
const $ = new Env('清空购物车');
...
let cookiesArr = [], cookie = '', postBody = '', venderCart, message = '';
//购物车内容
...
!(async () => {
  if ($.cart) {
    ...
    console.log("默认不清空购物车，如需：设置环境变量 JD_CART 为 true")
  }
  if (message) {
    $.msg($.name, '', message);
...
function getCart_xh() {
  ...
  return new Promise((resolve) => {
    const option = {
      ...
      body: `pingouchannel=0&commlist=123,,1,123,11,123,0,skuUuid:aaa@@useUuid:0&type=0&checked=0&locationid=&templete=1&reg=1&scene=0&version=20190418&traceid=1394319544881167891&tabMenuType=1&sceneval=2`,
      headers: {
      ...
      try {
        if (err) {
          ...
          if (data.errId === '0') {
            ...
            console.log(`获取到购物车数据成功，共： ${$.beforeRemove} 条`)
          }
        }
      } catch (e) {
        $.logErr(e, resp);
    ...
    $.post(option, async (err, resp, data) => {
      try {
        ...
        if (data.errId === '0') {
          ...
          console.log(`删除购物车${$.beforeRemove - $.afterRemove}件商品成功，当前购物车剩余数据 ${$.afterRemove} 条\n`)
          if ($.afterRemove - $.beforeRemove < 0) {
            message += `账号${$.index} ${$.UserName}\n成功删除购物车${$.beforeRemove - $.afterRemove}件商品\n\n`;
          }
        } else {
          console.log('删除失败，详情' + $.toStr(data))
